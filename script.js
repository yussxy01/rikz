const CONFIG = {
    storeName: "Rikz Store", 
    ownerName: "RIKZ", 
    avatar: "image/logo/logo.jpg", 
    instagram: "",
    telegram: "",
    whatsappNumber: "6283185965331", 
    adminNumber: "6283185965331"
};
...
const products = [
    ...
    {
        id: 3,
        name: "Suntik Followers Saluran",
        price: "6000",
        image: "image/produk/produk3.jpg",
        variants: [
            { label: "100 Followers", price: "6000" },
            { label: "200 Followers", price: "12000" },
            { label: "300 Followers", price: "15000" },
            { label: "400 Followers", price: "20000" },
            { label: "500 Followers", price: "25000" },
            { label: "600 Followers", price: "30000" },
            { label: "700 Followers", price: "35000" },
            { label: "800 Followers", price: "40000" },
            { label: "900 Followers", price: "45000" },
            { label: "1000 Followers", price: "50000" }
        ]
    },
    {
        id: 4,
    ...
    {
        id: 5,
        name: "Grafik MC/Minecraft (Khusus Game Free Fire)",
        price: "2000",
        image: "image/produk/produk5.jpg"
    },
    {
        id: 6,
        name: "Apk Autosave Kontak",
        price: "3000",
        image: "image/produk/produk6.jpg"
    },
    {
        id: 7,
        name: "Apk Badak WhatsApp",
        price: "5000",
...
function showProductDetail(product) {
    ...
    if (product.variants && product.variants.length > 0) {
        variantHtml = `
            <div style="margin-top:16px;">
                <div style="font-size:14px; color:#8e95a5; margin-bottom:8px;">Pilih Paket:</div>
                <select id="variantSelect" style="width:100%; padding:12px; border-radius:12px; background:#1a1d24; color:white; border:1px solid #2a2f38; font-size:14px;">
                    ${product.variants.map((v, i) => `
                        <option value="${i}">${v.label} - Rp ${parseInt(v.price).toLocaleString('id-ID')}</option>
                    `).join('')}
                </select>
            </div>
        `;
    ...
    content.innerHTML = `
        ...
        <div class="detail-price" id="detailPriceDisplay">Rp ${parseInt(product.price).toLocaleString('id-ID')}</div>
        ${variantHtml}
    ...
    if (variantSelect) {
        variantSelect.addEventListener("change", () => {
            const selected = product.variants[variantSelect.value];
            document.getElementById("detailPriceDisplay").innerText = `Rp ${parseInt(selected.price).toLocaleString('id-ID')}`;
        });
    }
    ...
    document.getElementById("detailBuyBtn").onclick = () => {
        ...
        if (variantSelect) {
            const selected = product.variants[variantSelect.value];
            selectedProduct.id = product.id + "-" + variantSelect.value;
            selectedProduct.name = `${product.name} - ${selected.label}`;
            selectedProduct.price = selected.price;
        }
...
function showPayment(product) {
    ...
    content.innerHTML = `
        <div style="text-align:center; padding: 0 10px;">
            ...
            <div style="font-size:28px; font-weight:800; color:#22d3ee; margin:12px 0;">Rp ${parseInt(product.price).toLocaleString('id-ID')}</div>
    ...
    document.getElementById("confirmWAButton").onclick = () => {
        const message = `Halo admin, saya ingin konfirmasi pembayaran untuk produk: ${product.name} dengan harga Rp ${parseInt(product.price).toLocaleString('id-ID')}. Terima kasih.`;
...
function showCart() {
    ...
    main.innerHTML = `
        <div style="padding:20px 0;">
            ...
            <div class="cart-items">
                ${cart.map(item => `
                    <div class="cart-item">
                        <div><strong>${item.name}</strong><br><small>Rp ${parseInt(item.price).toLocaleString('id-ID')}</small></div>
    ...
    document.getElementById("checkoutFromCart")?.addEventListener("click", () => {
        const totalPrice = cart.reduce((sum, i) => sum + (parseInt(i.price) * i.qty), 0);
    ...
    main.innerHTML = `
        <div class="profile-section">
            ...
            <div class="profile-info">
                ...
                <p><span class="status-badge"></span> Online</p>
